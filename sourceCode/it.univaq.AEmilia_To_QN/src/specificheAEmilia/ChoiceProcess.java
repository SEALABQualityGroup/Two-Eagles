 ...
 * ad una condizione tra piÃ¹ di un termine di processo.
 ...
 * </code>
 * </pre>
 * dove &lt;process_term_2_sequence&gt; è una sequenza di
 * almeno due termini di processo separati da virgole,
 * ognuno possibilmente preceduto da un'espressione
 * booleana che stabilisce la condizione sotto cui è disponibile.
 *
...
public class ChoiceProcess extends ProcessTerm {
	 ...
	 */
	public ChoiceProcess(Boolean condizione, ProcessTerm[] processes) {
		super(condizione);
	...
	public void print()
		...
		// l'array dei processi alternativi non può essere vuoto
			...
			if (getProcesses() != null && ((ChoiceProcess)pt).getProcesses() != null)
				...
				if (getProcesses().length == ((ChoiceProcess)pt).getProcesses().length)
					...
					for (int i = 0; i < getProcesses().length; i++)
						{
						ris = ris &&
						getProcesses()[i].equals(((ChoiceProcess)pt).getProcesses()[i]);
						}
					return ris;
					}
				else return false;
