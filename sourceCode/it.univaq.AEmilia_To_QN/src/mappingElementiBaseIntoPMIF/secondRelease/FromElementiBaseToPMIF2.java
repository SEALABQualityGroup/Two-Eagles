...
import mappingElementiBaseIntoPMIF.elementiPMIFConEquals.DemandServTypeEq;
 ...
 * di code in elementi PMIF. Genera le domande di servizio
 *
 * @author Mirko
 *
 */
public class FromElementiBaseToPMIF2 implements IFromElementiBaseToPmif {
	 ...
	 *
	 */
	public FromElementiBaseToPMIF2()
		{
		super();
	 ...
	 * @param type
	 */
	public FromElementiBaseToPMIF2(QNMTypeEq type)
		{
		super();
		this.type = type;
		}
	...
	private void mappingElementoBase(ProcessoServizioConBuffer processoServizioConBuffer)
		...
		try {
			...
			for (int i = 0; i < processoServizioConBuffer.getNumeroSorgenti(); i++)
				...
				// Si crea un elemento DemandServType come segue
				DemandServTypeEq demandServTypeEq = new DemandServTypeEq();
				// Si imposta WorkloadName uguale al nome del canale della classe di clienti
				// che si sta considerando
				demandServTypeEq.addWorkloadName(dataProcessoServizioSorgente.getClasse());
				// Si imposta ServerID uguale al nome del Server definito
				demandServTypeEq.addServerID(serverType.getName().asString());
				// imposta il DemandService uguale al tempo di servizio della classe di clienti che si
				// sta considerando moltiplicato il numero di visite per la classe di clienti 
				// che si sta considerando
				Double double1 = dataProcessoServizioSorgente.getTempoServizio();
				Double double2 = dataProcessoServizioSorgente.getNumberOfVisits();
				Double double3 = double1 * double2;
				demandServTypeEq.addServiceDemand(double3.toString());
				...
				if (dataProcessoServizioSorgente.getNumeroDestinazioni() == 0)
					...
					demandServTypeEq.addTransit(transitType);
					}
				...
				serviceRequestType.addDemandServiceRequest(demandServTypeEq);
				}
			}
		catch (Exception e)
			{
	...
	private void mappingElementoBase(ProcessoRoutingConBuffer processoRoutingConBuffer)
		...
		try {
			...
			// Si crea un elemento DemandServTime come segue
			DemandServTypeEq demandServTypeEq = new DemandServTypeEq();
			// Si imposta WorkloadName uguale al nome del canale della classe di clienti
			// che si sta considerando
			demandServTypeEq.addWorkloadName(processoRoutingConBuffer.getCanale());
			// Si imposta ServerID uguale al nome del Server definito
			demandServTypeEq.addServerID(serverType.getName().asString());
			// Si imposta ServiceDemand uguale a zero
			demandServTypeEq.addServiceDemand(new Double(0).toString());
